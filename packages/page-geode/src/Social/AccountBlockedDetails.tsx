...
function AccountBlockedDetails ({ className = '', onClear, outcome: { from, output, when } }: Props): React.ReactElement<Props> | null {
...
function ShowFeed(): JSX.Element {
        ...
        return(
          <div>
            ...
            <Table.Row>
              ...
              <h2> {t_strong('Number of Accounts Blocked: ')}{numBadge(count)}</h2>
      ...
      return(
        <div>
          <Card>{t('No Account Blocked Data')}</Card>
        </div>
      )
    }
}
...
`;
export default React.memo(AccountBlockedDetails);
